...
from ._acquisition_function import CoExBO_UCB, PiBO_UCB, CoExBO_UCB_Meta, CoExBO_H_Meta
...
class DuelingAcquisitionFunction(BaseDuelingAcquisitionFunction):
    ...
    def dueling(self, meta=False):
        ...
        """
        if meta:
            piucb = CoExBO_H_Meta(self.model, self.prior_pref, self.pref_pairwise, self.rf_clf, self.beta, self.gamma, pi_augment=False)
        else:
        ...
        if self.hallucinate:
            ...
            model_fantasy = self.hallucination(X_ucb)
            if meta:
                piucb = CoExBO_H_Meta(model_fantasy, self.prior_pref, self.pref_pairwise, self.rf_clf, self.beta, self.gamma, pi_augment=False)
            else:
