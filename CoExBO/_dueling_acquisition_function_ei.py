...
from ._acquisition_function import CoExBO_UCB, PiBO_UCB, CoExBO_UCB_Meta, CoExBO_UCB_Meta_EI
...
class BaseDuelingAcquisitionFunction:
    ...
    def optimize_function(self, acqf, meta=False):
        """
        ...
        if meta:
            X_next = acqf.forward([0])
        #X_next = torch.argmax(acqusition_fuction)
        else:
...
class DuelingAcquisitionFunction(BaseDuelingAcquisitionFunction):
    ...
    def dueling(self, meta=False):
        ...
        """
        if meta:
            #piucb = CoExBO_UCB_Meta(self.model, self.prior_pref, self.beta, self.gamma, pi_augment=False)
            piucb = CoExBO_UCB_Meta_EI(self.model, self.prior_pref, self.beta, self.gamma, pi_augment=False)
        else:
            piucb = CoExBO_UCB(self.model, self.prior_pref, self.beta, self.gamma, pi_augment=False)
        X_ucb = self.optimize_function(piucb, self.meta)
        if self.hallucinate:
            # if selected, we hallucinate the GP on X_bo
            model_fantasy = self.hallucination(X_ucb)
            if meta:
                #piucb = CoExBO_UCB_Meta(model_fantasy, self.prior_pref, self.beta, self.gamma, pi_augment=False)
                piucb = CoExBO_UCB_Meta_EI(model_fantasy, self.prior_pref, self.beta, self.gamma, pi_augment=False)
            else:
                piucb = CoExBO_UCB(model_fantasy, self.prior_pref, self.beta, self.gamma, pi_augment=True)
        else:
            # otherwise not.
            piucb.pi_augment = True
        X_pref = self.optimize_function(piucb, self.meta)
    ...
    def __call__(self):
        ...
        if self.method == "ts":
            ...
            X_suggest = self.dueling(meta=self.meta)
        else:
